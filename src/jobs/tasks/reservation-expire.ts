...
export const reservationExpire: Task = async (_payload, _helpers) => {
  const log = logger.child({ component: RESERVATION_EXPIRE_TASK })
  ...
  const rows = await migratorPool<
    Array<{ id: string; tenant_id: string; lot_id: string; event_id: string }>
  >`
  ...
  for (const row of rows) {
    try {
      await withTenant(row.tenant_id, async (db) => {
        ...
        await emitOutboxEvent(db, 'lot.released', row.lot_id, {
      ...
      log.error(
        { reservationId: row.id, tenantId: row.tenant_id, error: err },
        'reservation.expire: failed to release reservation row',
      )
    }
  }
