...
const lotNotifyChannelSchema = z.object({
  tenant_id: z.string().uuid(),
  event_id: z.string().uuid(),
  lot_id: z.string().uuid(),
  new_status: z.enum(['available', 'reserved', 'sold', 'released']),
})

export type LotNotifyChannelPayload = z.infer<typeof lotNotifyChannelSchema>

// ---------------------------------------------------------------------------
// Task handler
// ---------------------------------------------------------------------------

const log = logger.child({ task: LOT_NOTIFY_CHANNEL_TASK })
 ...
 */
export const lotNotifyChannel: Task = async (rawPayload, _helpers) => {
  ...
  await migratorPool`SELECT pg_notify(${channel}, ${payloadJson})`
