...
export const cnpjFormattedRegex = /^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/

/** Matches 14 raw digits exactly. */
export const cnpjDigitsRegex = /^\d{14}$/
...
export function redactCNPJ(cnpj: string): string {
  const d = normalizeCNPJ(cnpj)
 ...
 */
export function validateCheckDigits(cnpj: string): boolean {
