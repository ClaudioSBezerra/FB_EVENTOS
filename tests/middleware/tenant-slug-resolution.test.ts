...
describe('SYSTEM_PREFIXES + slugReserved', () => {
  test('all 19 prefixes are present', () => {
    expect(SYSTEM_PREFIXES.size).toBe(19)
    ...
    expect(SYSTEM_PREFIXES.has('select-org')).toBe(true)
    expect(SYSTEM_PREFIXES.has('no-access')).toBe(true)
  })
