...
exports.getPagarmeBaseUrl = getPagarmeBaseUrl;
exports.getPagarmeEnv = getPagarmeEnv;
...
async function createOrder(payload, idempotencyKey) {
    ...
    if (!res.ok) {
        ...
        throw new types_1.PagarmeApiError(res.status, text);
    }
    const json = await res.json();
    return types_1.pagarmeOrderResponseSchema.parse(json);
}
// ────────────────────────────────────────────────────────────────────────────
 ...
 * @throws PagarmeApiError on non-2xx response.
 */
async function cancelCharge(chargeId, opts) {
    ...
    const json = await res.json();
    return types_1.pagarmeRefundResponseSchema.parse(json);
}
