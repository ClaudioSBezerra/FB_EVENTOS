...
exports.getZapsignBaseUrl = getZapsignBaseUrl;
exports.createDocument = createDocument;
exports.getDocument = getDocument;
exports.downloadSignedPdf = downloadSignedPdf;
...
function getToken() {
    ...
    if (!t)
        throw new types_1.ZapsignNotConfiguredError();
    return t;
}
// ────────────────────────────────────────────────────────────────────────────
// createDocument
...
async function createDocument(payload) {
    ...
    if (!res.ok) {
        ...
        throw new types_1.ZapsignApiError(res.status, text);
    }
    const json = await res.json();
    return types_1.zapsignCreateDocResponseSchema.parse(json);
}
// ────────────────────────────────────────────────────────────────────────────
 ...
 */
async function getDocument(zapsignToken) {
    const token = getToken();
 ...
 */
async function downloadSignedPdf(zapsignToken) {
    const doc = await getDocument(zapsignToken);
    if (!doc.signed_file) {
        throw new types_1.ZapsignApiError(409, `Document ${zapsignToken} has no signed_file (status=${doc.status})`);
    }
