...
exports.MAX_SSE_CONN = void 0;
exports.reservePgListenConnection = reservePgListenConnection;
const postgres_1 = __importDefault(require("postgres"));
const logger_1 = require("@/lib/logger");
const log = logger_1.logger.child({ module: 'sse:listen-pool' });
// ---------------------------------------------------------------------------
// Cap
// ---------------------------------------------------------------------------
/**
 ...
 */
exports.MAX_SSE_CONN = Number(process.env.MAX_SSE_CONN ?? '200');
let activeCount = 0;
// ---------------------------------------------------------------------------
// Factory
// ---------------------------------------------------------------------------
 ...
 */
async function reservePgListenConnection() {
    if (activeCount >= exports.MAX_SSE_CONN) {
        throw new Error(`SSE connection cap reached (MAX_SSE_CONN=${exports.MAX_SSE_CONN}). ` +
            'Try again later or contact the operator to raise the cap.');
    }
    const url = process.env.DATABASE_URL;
    if (!url)
        throw new Error('DATABASE_URL is not set');
    const conn = (0, postgres_1.default)(url, {
        max: 1,
        idle_timeout: 0,
        connection: {
