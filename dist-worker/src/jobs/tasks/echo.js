...
const echo = async (payload, helpers) => {
    ...
    const p = (payload ?? {});
    const log = (0, logger_1.childLogger)({
    ...
    if (!p.requestId) {
        logger_1.logger.debug({ component: 'job', task: 'echo', jobId: String(helpers.job.id) }, 'echo (no requestId)');
    }
};
exports.echo = echo;
